...
import com.homework.web.pojo.ChapterNameDTO;
import com.homework.web.service.ChapterService;
import com.homework.web.util.PathUtil;
...
public class ChapterController {
    ...
    public ResponseObject post(@RequestBody HashMap<String, String> data) {
        ...
        if (data.get("title") == null || data.get("title").equals("")) {
            ...
            try {
                // 文件夹
                File directory = new File(PathUtil.getClassLoadRootPath() + "/src/main/resources/static/txt/");
                if (!directory.exists()) {
                    directory.mkdirs();
                }
                // 文件
    ...
    public ResponseObject getContent(String content) {
        ...
        if (content == null || content.equals("")) {
            ...
            try {
                ...
                File file = new File(new File(PathUtil.getClassLoadRootPath() + "/src/main/resources/static/txt/"),
                        content);
    ...
    @ApiOperation("修改Chapter的name")
//	@PreAuthorize("isAuthenticated()")
    public ResponseObject updateName(@RequestBody ChapterNameDTO chapterNameDTO) {
        log.info("修改Chapter的content");

        if (chapterNameDTO.getChapterId() == null) {
            throw new ControllerException("chapterId 不能为空");
        }
        chapterService.updateName(chapterNameDTO);
        return new ResponseObject("200", "操作成功", chapterNameDTO.getChapterTitle());
