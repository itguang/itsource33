  ...
  <div>
    <el-row>
      ...
      <el-col :span="8" :offset="2">
        ...
        <el-button type="text" v-if="$store.state.user != null && $store.state.user.id == id" @click="dialogVisible = true;userForm.nickname = $store.state.user.nickname;userForm.profile = $store.state.user.profile;userForm.phone = $store.state.user.phone;userForm.email = $store.state.user.email;">编辑</el-button>
      </el-col>
      <!-- 靠右-->
      <div>
        <el-button  type="success"  v-if="$store.state.user != null && $store.state.user.id == id" @click="$router.push('/home/novelManagement')">作品管理</el-button>
      </div>
    </el-row>
    <el-row>
              ...
              <el-row :gutter="20" v-for="(item,i) in  novelList" :key="i">
                ...
                <el-col :span="21">
                  <el-tooltip :content="getNovelData(item.id)" placement="top" effect="light">
                    <div >
                      <el-link type = 'primary' :underline="false"><h1 @click="$router.push({path: '/home/novel',query: {id: item.id}})">{{item.name}}</h1></el-link>
                      <h5>{{item.summary}}</h5>
                    </div >
                  </el-tooltip>
                </el-col>
          ...
          <el-tab-pane :label="'粉丝' + followerUserList.length" name="fan">
            <el-card class="box-card">
              <el-row :gutter="20" v-for="(item,i) in followerUserList" :key="i">
                ...
                <el-divider type = 'success'></el-divider>
              </el-row>
            </el-card>
          </el-tab-pane>
        </el-tabs>
