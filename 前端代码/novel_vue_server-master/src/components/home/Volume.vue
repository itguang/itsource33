<template>
  <div>
    ...
    <el-divider>卷名称</el-divider>
    <el-row>
      <el-col :span="16"> {{volume.name}}</el-col>
      <el-col :span="8"><el-button type="text" @click="dialogVisibleOfName = true;dialogDataOfName = volume.name">修改</el-button></el-col>
    </el-row>
    <el-divider>卷摘要</el-divider>
    <el-row>
      <el-col :span="16">{{volume.summary}}</el-col>
      <el-col :span="8"><el-button type="text" @click="dialogVisibleOfSummary = true;dialogDataOfSummary = volume.summary">修改</el-button></el-col>
    </el-row>
    <el-divider> 卷章节</el-divider>
    <el-button type="primary" @click="newChapterVisible = true">添加章节</el-button>
    <el-divider> 章节列表</el-divider>
    <el-row v-for="(item,i) in chapterList" :key="i">
      <el-col :span="12">{{item.title}}</el-col>
      <el-col :span="12">
        <el-button type="text" @click="$router.push({path: '/home/chapter',query: {id: item.id}})">查看</el-button>
        <el-button type="text" @click="updateChapterNameDialog = true;chapterId = item.id;chapterTitle = item.title">修改</el-button>
      </el-col>
    </el-row>

    <el-dialog title="请输入新的 章节名称" :visible.sync="updateChapterNameDialog" width="21%">
      <el-form :inline="true" class="demo-form-inline">
        <el-form-item>
          <el-input v-model="chapterTitle"></el-input>
        </el-form-item>
        <el-form-item>
          <el-button type="primary" @click="updateChapterName(chapterId,chapterTitle)">修改</el-button>
        </el-form-item>
      </el-form>
    </el-dialog>
    ...
    <el-dialog title="请输入新的name" :visible.sync="dialogVisibleOfName" width="21%">
      <el-form :inline="true" class="demo-form-inline">
        ...
        <el-form-item>
          <el-button type="primary" @click="patchName">确定</el-button>
        </el-form-item>
      </el-form>
    </el-dialog>
...
<script>
import { userRequest,post } from '@/network/userRequest'
export default {
  name: 'Volume',
  data: function () {
    return {
  ...
  methods: {
    ...
    updateChapterName (chapterId, chapterTitle) {
      this.updateChapterNameDialog = false
      ...
      const response  =   post('/api/chapter/updateName',body).then(res => {
